...
function empurra(w,h)
	...
	local retangulo = nil
	function empurrarObjetosDinamicos(toque)
		...
		if toque.phase == "began" then
			...
			retangulo = display.newRect(toque.x,toque.y,w,h)
			...
			-- ao colocar esse foco, evitamos que o programa ative outro evento ou desative esse mesmo 
			...
			display.getCurrentStage():setFocus(toque.target, toque.id);
		elseif toque.phase == "moved" then
			-- aqui atualizamos a posição do retângulo toda vez que o dedo se mover.
			retangulo.x = toque.x
			retangulo.y = toque.y
		elseif toque.phase == "ended" then
